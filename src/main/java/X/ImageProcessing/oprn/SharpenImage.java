...
public class SharpenImage {
	private static final Logger logger = LoggerFactory.getLogger(SharpenImage.class);
...
// USER-CODE-BLOCK BEGIN -- Do not modify anything outside this block or this comment line.
		...
		try {
			...
			Mat source;
			// = Imgcodecs.imread(f.getAbsolutePath(), Imgcodecs.IMREAD_COLOR);
			source = Imgcodecs.imdecode(new MatOfByte(imageData_in), Imgcodecs.IMREAD_UNCHANGED);
			Mat destination = new Mat(source.rows(), source.cols(), source.type());
			Imgproc.GaussianBlur(source, destination, new Size(0, 0), 10);
			Core.addWeighted(source, 1.5, destination, -0.5, 0, destination);

			// Imgcodecs.imwrite(f.getAbsolutePath(), destination);
